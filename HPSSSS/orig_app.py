...
def predict_disease(user_responses,current_question_list):
    ...
    if current_question_list == questions["heart_disease"]:
        ...
        if probability_of_positive_class >= threshold:
            ...
            return f"Our prediction suggests a potential risk of heart disease of {probability:.2f}%. Maintain your well-being with regular check-ups. Move ahead to generate a report!"
        else:
            return f"Our prediction suggests a low risk of heart disease {probability:.2f}% . Maintain your well-being with regular check-ups. Move ahead to generate a report!"
    ...
    # Example user input
        ...
        scaler.fit(X_train)  # Replace X_train with the actual training data
    ...
    # Calculate accuracy using the appropriate variables (y_test and y_pred_rf)
        ...
        if probability_of_positive_class >= threshold_pad:
            return f"Our prediction suggests a potential risk of Peripheral Artery Disease of {probability_pad:.2f}%. Respond furthur to generate a Report"
        else:
            return f"Our prediction suggests a low risk of Peripheral Artery Disease {probability_pad:.2f}%. Respond furthur to generate a Report"
